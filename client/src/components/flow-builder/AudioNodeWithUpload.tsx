...
import { FileAudio, Copy, Trash2 } from 'lucide-react';
...
export function AudioNodeWithUpload({ id, data, isConnectable }: AudioNodeProps) {
  const { t } = useTranslation();
  const [isEditing, setIsEditing] = useState(false);
  const [mediaUrl, setMediaUrl] = useState(data.mediaUrl || "");
  const [caption, setCaption] = useState(data.caption || t('flow_builder.audio_upload_default_caption', 'Listen to this audio!'));
      ...
      xhr.onload = () => {
        ...
        if (xhr.status === 200) {
          ...
          toast({
            ...
            description: t('flow_builder.audio_upload_upload_success', 'Audio uploaded successfully')
          });
        } else {
          toast({
            title: t('flow_builder.audio_upload_upload_failed', 'Upload failed'),
            description: t('flow_builder.audio_upload_upload_error', 'There was an error uploading your audio'),
            variant: 'destructive'
          });
        }
      };
  ...
  return (
    <div className="node-audio p-3 rounded-lg bg-white border border-border shadow-sm max-w-[250px] relative group">
        ...
        <div className="absolute -top-8 -right-2 bg-background border rounded-md shadow-sm flex z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent side="top">
                <p className="text-xs">{t('flow_builder.audio_upload_duplicate_node', 'Duplicate node')}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent side="top">
                <p className="text-xs">{t('flow_builder.audio_upload_delete_node', 'Delete node')}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
      ...
      <div className="font-medium flex items-center gap-2 mb-2">
        ...
        <span>{t('flow_builder.audio_upload_node_title', 'Audio Message')}</span>
        <button 
      ...
      {isEditing ? (
        <div className="space-y-3">
          <div>
            <p className="text-xs font-medium mb-1">{t('flow_builder.audio_upload_upload_label', 'Upload Audio:')}</p>
            <FileUpload
          ...
          <div>
            <p className="text-xs font-medium mb-1">{t('flow_builder.audio_upload_caption_label', 'Caption:')}</p>
            <input
              className="w-full p-2 text-sm border rounded"
              value={caption}
              onChange={handleCaptionChange}
              placeholder={t('flow_builder.audio_upload_caption_placeholder', 'Enter caption')}
            />
          </div>
        </div>
      ) : (
            ...
            <div className="relative bg-secondary/30 rounded overflow-hidden p-3 flex flex-col items-center justify-center">
              ...
              {mediaUrl.match(/\.(mp3|wav|ogg|m4a)$/i) && (
                <audio controls className="w-full max-w-full">
                  <source src={mediaUrl} />
                  {t('flow_builder.audio_upload_browser_not_supported', 'Your browser does not support the audio element.')}
                </audio>
              )}
